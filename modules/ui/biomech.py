...
def render_biomech_tab(df_plot, df_plot_resampled):
    ...
    if is_running:
            ...
            if cad_stats.get("mean_spm", 0) > 0:
                ...
                if time_col in df_plot.columns:
                    ...
                    if valid_mask.sum() > 100:
                        try:
                            slope, intercept, _, _, _ = stats.linregress(
                                df_plot.loc[valid_mask, time_col], df_plot.loc[valid_mask, cad_col]
                            )
                    ...
                    time_vals_cad = (
                        ...
                        if hasattr(df_plot[time_col], "values")
                        else np.array(df_plot[time_col])
                    )
                    tick_step_cad = 5
                    tick_vals_cad = np.arange(0, time_vals_cad.max() + tick_step_cad, tick_step_cad)
                    tick_text_cad = [
                        f"{int(m // 60):02d}:{int(m % 60):02d}:00" for m in tick_vals_cad
                    ]
        ...
        if gct_col:
            ...
            if gct_stats.get("mean_ms", 0) > 0:
                ...
                if time_col in df_plot.columns:
                    ...
                    fig_gct.add_hrect(
                        y0=200, y1=220, fillcolor="limegreen", opacity=0.1, line_width=0
                    )
                    ...
                    tick_text_gct = [
                        f"{int(m // 60):02d}:{int(m % 60):02d}:00" for m in tick_vals_gct
                    ]
        ...
        if "stance_time_balance" in df_plot.columns:
            ...
            if len(balance_data) > 0:
                ...
                if imbalance < 1.0:
                    ...
                    b4.metric(
                        "Asymetria", f"{imbalance:.1f}%", delta="Uwaga", delta_color="inverse"
                    )
                ...
                if time_col in df_plot.columns:
                    ...
                    fig_bal.add_hrect(
                        y0=49.0, y1=51.0, fillcolor="green", opacity=0.1, line_width=0
                    )
        ...
        has_real_step = (
            "step_length" in df_plot.columns and df_plot["step_length"].notna().sum() > 10
        )
        ...
        if has_real_step or ("cadence" in df_plot.columns and "pace" in df_plot.columns):
            ...
            if has_real_step:
                ...
                valid_mask = (
                    (df_plot["cadence"] > 50) & (df_plot["cadence"] < 300) & (df_plot["pace"] > 0)
                )
                df_stride = df_plot[valid_mask].copy()
            ...
            if not df_stride.empty and "step_length" in df_stride.columns:
                ...
                # Convert time to hh:mm:ss format for x-axis
                time_vals_stride = (
                    df_stride[time_col].values
                    if hasattr(df_stride[time_col], "values")
                    else np.array(df_stride[time_col])
                )
                tick_step_stride = 5
                tick_vals_stride = np.arange(
                    0, time_vals_stride.max() + tick_step_stride, tick_step_stride
                )
                tick_text_stride = [
                    f"{int(m // 60):02d}:{int(m % 60):02d}:00" for m in tick_vals_stride
                ]
        ...
        if "watts" in df_plot.columns and "pace" in df_plot.columns:
            ...
            if not df_re.empty:
                ...
                re1.metric(
                    "Średnie RE", f"{avg_re:.3f} m/s/W/kg", help="RE = prędkość (m/s) / moc (W/kg)"
                )
                ...
                if time_col in df_re.columns:
                    ...
                    if len(valid_re) > 100:
                        ...
                        total_drift = slope * (
                            valid_re[time_col].iloc[-1] - valid_re[time_col].iloc[0]
                        )
                        re2.metric(
                            "Trend RE",
                            f"{total_drift:+.3f}",
                ...
                fig_re.add_hline(
                    y=1.0, line_dash="dash", line_color="green", annotation_text="RE = 1.0 (dobre)"
                )

                # Convert time to hh:mm:ss format for x-axis
                time_vals_re = (
                    df_re[time_col].values
                    if hasattr(df_re[time_col], "values")
                    else np.array(df_re[time_col])
                )
                tick_step_re = 5
...
def _render_vertical_oscillation_section(df_plot, df_plot_resampled):
    ...
    if time_col in df_plot.columns:
        ...
        if valid_mask.sum() > 100:
            ...
            try:
                slope, intercept, _, _, _ = stats.linregress(
                    df_plot.loc[valid_mask, time_col], df_plot.loc[valid_mask, vo_col]
                )
                trend = intercept + slope * df_plot[time_col]
                fig_vo.add_trace(
                    go.Scatter(
    ...
    if cad_col:
        ...
        if efficiency.get("optimal_cadence"):
            st.success(
                f"🎯 **Optymalna kadencja:** {efficiency['optimal_cadence']} SPM "
