...
def _render_trigger_button(df_clean_pl: Any) -> None:
    """Render the DFA computation trigger UI when no results are cached."""
...
def _interpolate_columns(df_dfa: Any, df_clean_pl: Any) -> Any:
    """Add interpolated watts/hr and time_min columns to df_dfa."""
    ...
    return df_dfa, df_clean


def _render_summary_metrics(df_dfa: Any) -> None:
    """Render RMSSD, SDNN, RR, and HR metrics row."""
...
def _make_time_tick_labels(time_vals: np.ndarray, tick_step: int = 5) -> tuple:
    """Return (tick_vals, tick_text) for hh:mm:ss formatted x-axis."""
    vals = np.arange(0, time_vals.max() + tick_step, tick_step)
    text = [f"{int(m // 60):02d}:{int(m % 60):02d}:00" for m in vals]
    return vals, text


def _render_dfa_chart(df_dfa: Any) -> None:
    """Render the main DFA Alpha-1 vs time chart."""
    ...
    tick_vals, tick_text = _make_time_tick_labels(time_vals)
...
def _render_rmssd_chart(df_dfa: Any) -> None:
    """Render the RMSSD time-series chart."""
    if "rmssd" not in df_dfa.columns:
        return
...
def _render_poincare_plot(df_clean: Any) -> None:
    """Render the Poincaré (Lorenz) plot from raw RR intervals."""
    st.markdown("---")
    ...
    sd2 = float(np.sqrt(2 * np.std(raw_rr_series) ** 2 - 0.5 * np.std(diff_rr) ** 2))
    ...
    min_rr, max_rr = float(min(raw_rr_series)), float(max(raw_rr_series))
    fig_poincare.add_trace(
        go.Scatter(
            x=[min_rr, max_rr],
            y=[min_rr, max_rr],
...
def _render_ddfa_and_thresholds(df_dfa: Any) -> None:
    """Render DDFA trend analysis and HRV threshold detection."""
    st.markdown("---")
    st.subheader("🔬 Zaawansowana analiza: DDFA i Progi HRV")

    if "alpha1" not in df_dfa.columns or len(df_dfa) <= 10:
        return
...
def _render_hrv_thresholds(df_dfa: Any) -> None:
    """Detect and render HRVT1/HRVT2 threshold metrics."""
    if "hr" not in df_dfa.columns and "watts" not in df_dfa.columns:
        return
    ...
    if not thresholds.get("hrvt1_intensity"):
        st.info(
...
def _render_no_data_section(df_clean_pl: Any, dfa_error: Any) -> None:
    """Render diagnostics and guidance when DFA data is unavailable."""
    df_clean = df_clean_pl.to_pandas() if hasattr(df_clean_pl, "to_pandas") else df_clean_pl
    hrv_cols = [
        c for c in df_clean.columns if any(x in c.lower() for x in ["rr", "hrv", "ibi", "r-r"])
    ]
...
def render_hrv_tab(df_clean_pl: Any) -> None:
    ...
    if st.session_state.df_dfa is None:
        _render_trigger_button(df_clean_pl)
    ...
    if df_dfa is not None and not df_dfa.empty:
        ...
        df_dfa, df_clean = _interpolate_columns(df_dfa, df_clean_pl)

        _render_summary_metrics(df_dfa)
        _render_dfa_chart(df_dfa)
        _render_rmssd_chart(df_dfa)
        _render_poincare_plot(df_clean)
        _render_ddfa_and_thresholds(df_dfa)
        ...
        _render_no_data_section(df_clean_pl, dfa_error)
