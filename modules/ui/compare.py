...
def render_comparison_tab(target_analysis_result: dict = None):
    ...
    with st.expander("➕ Dodaj Sesję Historyczną", expanded=True):
        ...
        if st.button("Dodaj do Historii"):
            new_entry = {}
            if h_vt1 > 0:
                new_entry["VT1_Watts"] = h_vt1
            if h_vt2 > 0:
                new_entry["VT2_Watts"] = h_vt2
            if h_tau > 0:
                new_entry["SmO2_Tau"] = h_tau
            if h_hr_vt1 > 0:
                new_entry["HR_VT1"] = h_hr_vt1
            if h_hr_vt2 > 0:
                new_entry["HR_VT2"] = h_hr_vt2
            if h_vo2 > 0:
                new_entry["VO2max"] = h_vo2
    ...
    if st.session_state.history_metrics:
        ...
        for metric, stats in baseline_stats.items():
            current_val = (
                target_analysis_result.get(metric, None) if target_analysis_result else None
            )
            ...
            if current_val is not None:
                ...
                details.get("status", "Stable")

                row["Diff (%)"] = f"{pct_diff:+.1f}%"

                if is_sig:
                    status_emoji = (
                        "⚠️ Significant Change" if abs(pct_diff) > 5 else "❗ Possible Change"
                    )
                else:
                    status_emoji = "✅ Stable"
        ...
        if unstable_metrics:
            st.error(
                f"**Uwaga - Niska Powtarzalność Metryk:** {', '.join(unstable_metrics)}. Te wskaźniki mają wysoką zmienność (CV > 10%) i mogą być niewiarygodne do sterowania obciążeniem."
            )
        else:
        ...
        if changed_metrics:
            st.warning(
                f"**Wykryto zmianę adaptacyjną:** {', '.join(changed_metrics)}. Wartość bieżąca odbiega od bazy w sposób istotny statystycznie."
            )
