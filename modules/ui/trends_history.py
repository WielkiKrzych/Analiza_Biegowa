...
def render_trends_history_tab():
    ...
    with col1:
        days = st.selectbox(
            "Zakres", options=[30, 60, 90, 180, 365], index=2, format_func=lambda x: f"{x} dni"
        )
    ...
    df_weekly = (
        df.set_index("date")
        .resample("W")
        .agg({"tss": "sum", "np": "mean", "avg_watts": "mean", "duration": "sum"})
        .reset_index()
    )
    ...
    with col1:
        ...
        if df["mmp_5m"].notna().sum() > 2:
            ...
            fig_5m.update_layout(
                template="plotly_dark", height=250, title="5-min MMP", yaxis_title="Watts"
            )
    ...
    with col2:
        ...
        if df["mmp_20m"].notna().sum() > 2:
            ...
            fig_20m.update_layout(
                template="plotly_dark", height=250, title="20-min MMP (≈FTP)", yaxis_title="Watts"
            )
    ...
    if df["avg_rmssd"].notna().sum() > 2:
        ...
        fig_hrv.update_layout(template="plotly_dark", height=300, yaxis_title="RMSSD (ms)")
