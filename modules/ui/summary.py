...
# =============================================================================
# SECTION 8-12: NEW ANALYSIS SECTIONS
# =============================================================================
...
def _render_durability_section(df_plot):
    ...
    dec_colors = {"excellent": "#27AE60", "good": "#2ECC71", "moderate": "#F39C12", "poor": "#E74C3C"}
    ...
    if "ef_series" in onset and onset["ef_series"] is not None:
        ...
        if len(ef_series) > 60:
            ...
            fig_ef.update_layout(
                ...
                xaxis_title="Czas [min]", yaxis_title="EF (speed/HR)",
                margin=dict(l=20, r=20, t=30, b=20),
            )
...
def _render_br_analysis_section(df_plot):
    ...
    fig_br = go.Figure(data=[go.Bar(
        x=zone_names, y=zone_values,
    ...
    if vt_result.get("vt1_br") is not None:
        ...
        c2.metric("VT2 (BR)", f"{vt_result.get('vt2_br', 0):.0f} /min" if vt_result.get("vt2_br") else "--")
    ...
    if "heartrate" in df_plot.columns:
        ...
        st.caption(f"BR:HR Decoupling: {br_dec['decoupling_pct']:.1f}% — {br_dec.get('interpretation', '')}")
...
def _render_thermal_analysis_section(df_plot):
    ...
    if time_col is not None:
        ...
        fig_temp.update_layout(
            ...
            xaxis_title="Czas [min]", yaxis_title="Core Temperature [°C]",
...
def _render_running_effectiveness_section(df_plot, rider_weight):
    ...
    if "watts" in df_plot.columns and ("velocity_smooth" in df_plot.columns or "pace" in df_plot.columns):
        has_any = True
