...
def render_nutrition_tab(df_plot, critical_pace, vt1_pace, vt2_pace):
    ...
    is_running = sport_type == "running" or (
        "pace" in df_plot.columns and "watts" not in df_plot.columns
    )

    # Interaktywne suwaki
    c1, c2, c3 = st.columns(3)
    carb_intake = c1.number_input(
        "Spożycie Węglowodanów [g/h]", min_value=0, max_value=200, value=60, step=10
    )
    initial_glycogen = c2.number_input(
        "Początkowy Zapas Glikogenu [g]",
        min_value=200,
        max_value=800,
        value=450,
        step=50,
        help="Standardowo: 400-500g dla wytrenowanego sportowca.",
    )
    c3.number_input(
    ...
    if "watts" in df_plot.columns:
        ...
        intensity = (
            df_plot["watts"] / critical_pace if critical_pace > 0 else df_plot["watts"] / 280
        )
        ...
        cho_fraction = np.where(
            intensity < 0.55,
            0.30,
            np.where(
                intensity < 0.75,
                0.30 + (intensity - 0.55) * 1.5,  # 30→60%
                np.where(
                    intensity < 1.00,
                    0.60 + (intensity - 0.75) * 1.2,  # 60→90%
                    np.clip(0.90 + (intensity - 1.0) * 0.5, 0.90, 1.0),
                ),
            ),
        )  # 90-100%
        ...
        burn_rate_smooth = (
            df_nutri["Burn Rate [g/h]"].rolling(window=60, center=True, min_periods=1).mean()
        )
        ...
        fig_burn.add_hline(
            ...
            line_dash="dot",
            line_color="#00cc96",
            annotation_text=f"Intake: {carb_intake}g/h",
        ...
        n1.metric(
            "Spalone Węgle", f"{total_burn:.0f} g", help="Suma węglowodanów zużytych na wysiłek"
        )
        n2.metric(
            "Spożyte Węgle",
            f"{total_intake:.0f} g",
            help="Suma węglowodanów dostarczonych z jedzenia/napojów",
        )
        n3.metric(
            "Wynik Końcowy",
            f"{final_balance:.0f} g",
            delta=f"{final_balance - initial_glycogen:.0f} g",
            delta_color="inverse" if final_balance < 0 else "normal",
        )
        n4.metric(
            "Śr. Spalanie",
            f"{avg_burn_rate:.0f} g/h",
            help="Średnie tempo spalania węgli podczas treningu",
        )
        ...
        if final_balance < 0:
            ...
            st.error(
                f"⚠️ **UWAGA:** Według symulacji, Twoje zapasy glikogenu wyczerpały się w okolicach {bonk_time:.0f} minuty! To oznacza ryzyko 'odcięcia' (bonk)."
            )
        else:
            st.success(
                f"✅ **OK:** Zakończyłeś trening z zapasem {final_balance:.0f}g glikogenu. Strategia żywieniowa wystarczająca dla tej intensywności."
            )
        ...
        cho_fraction = np.where(
            ...
            np.where(
                intensity < 0.80,
                0.50 + (intensity - 0.60) * 0.75,
                np.where(
                    intensity < 1.00,
                    0.65 + (intensity - 0.80) * 0.75,
        ...
        n4.metric(
            "Śr. Spalanie",
            f"{avg_burn_rate:.0f} g/h",
            help="Średnie tempo spalania węgli podczas biegu",
        )
        ...
        if final_balance < 0:
            ...
            st.success(
                f"✅ **OK:** Zakończyłeś bieg z zapasem {final_balance:.0f}g glikogenu. Strategia żywieniowa wystarczająca dla tej intensywności."
            )
        ...
        st.warning(
            "Brak danych mocy (Watts) lub tempa (Pace) do obliczenia wydatku energetycznego."
        )
