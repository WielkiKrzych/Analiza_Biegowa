...
def _build_thermal_chart(df_plot) -> Optional[go.Figure]:
    ...
    fig.add_hline(
        ...
        line_color="red",
        opacity=0.5,
        annotation_text="Krytyczna (38.5°C)",
    ...
    fig.add_hline(
        ...
        line_color="green",
        opacity=0.5,
        annotation_text="Optymalna (37.5°C)",
    ...
    time_vals = (
        df_plot["time_min"].values
        if hasattr(df_plot["time_min"], "values")
        else np.array(df_plot["time_min"])
    )
    ...
    fig.update_layout(
        ...
        yaxis2=dict(
            title="HSI [0-10]", overlaying="y", side="right", showgrid=False, range=[0, 12]
        ),
...
def _render_thermal_kpi(decay_res: dict) -> None:
    ...
    if decay_res["r_squared"] > 0:
        ...
        col1.metric(
            "Koszt Termiczny",
            f"{decay_res['decay_pct_per_c']}% / 1°C",
        ...
        status = (
            ...
            else ("🟡 Średni" if decay_res["decay_pct_per_c"] < -3 else "🟢 Niski")
        )
