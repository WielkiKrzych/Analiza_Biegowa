...
def render_vent_tab(target_df, training_notes, uploaded_file_name):
    ...
    # BR-only path: show BR zone analysis when VE is absent
    if not has_ve and has_br:
        ...
        st.caption("Dane z zegarka (Garmin/COROS). Brak pełnej wentylacji (VE) — analiza oparta wyłącznie na BR.")
        ...
        if len(br_series) > 60:
            ...
            if "time" in target_df.columns:
                ...
                fig_br_ts = go.Figure()
                fig_br_ts.add_trace(go.Scatter(
                    x=time_min, y=br_smooth.values,
                    name="BR (oddechów/min)", line=dict(color="#3498db", width=2),
                ))
                fig_br_ts.update_layout(
                    template="plotly_dark",
                    title="Częstość oddechów w czasie",
                    xaxis_title="Czas [min]", yaxis_title="BR [oddechów/min]",
