...
class EnvironmentService:
    ...
    def get_conditions(self, date: datetime, lat: float, lon: float) -> Optional[WeatherData]:
    ...
    def calculate_tss_correction(self, weather: WeatherData) -> Tuple[float, str]:
        ...
        for (min_t, max_t), factor in self.TEMP_CORRECTION.items():
            if min_t <= weather.temperature < max_t:
                ...
                explanations.append(
                    f"Temperatura {weather.temperature:.0f}°C: +{factor * 100:.0f}%"
                )
                break
    ...
    def adjust_tss(self, base_tss: float, weather: WeatherData) -> Tuple[float, str]:
    ...
    def get_heat_acclimation_status(self, recent_hot_sessions: int) -> str:
