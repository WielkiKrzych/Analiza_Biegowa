...
def estimate_vlamax(
    cp_watts: float, w_prime_kj: float, pmax_watts: float, weight_kg: float = 75
) -> float:
    """
...
def classify_phenotype(vo2max: float, vlamax: float, anaerobic_reserve_pct: float) -> str:
    """
    ...
    vo2max / vlamax if vlamax > 0 else 0
...
def diagnose_limiter(vo2max: float, vlamax: float, cp_watts: float, weight_kg: float) -> tuple:
    """
    ...
    cp_watts / weight_kg if weight_kg > 0 else 0
...
def generate_training_block(profile: MetabolicProfile, weeks: int = 6) -> TrainingBlock:
    """
