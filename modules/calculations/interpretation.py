...
def interpret_results(
    ...
    if thresholds is not None:
        ...
        if vt1_zone is not None and hasattr(vt1_zone, "confidence"):
            if vt1_zone.confidence < 0.7:
                result.uncertainties.append(f"❓ VT1 niska pewność ({vt1_zone.confidence:.0%})")
        ...
        if vt2_zone is not None and hasattr(vt2_zone, "confidence"):
            if vt2_zone.confidence < 0.7:
                result.uncertainties.append(f"❓ VT2 niska pewność ({vt2_zone.confidence:.0%})")
    ...
    if overall_quality < 0.5:
        ...
        result.data_quality_note = f"⚠️ Średnia jakość danych. {', '.join(quality_issues)}"
    else:
    ...
    if vt1_watts and vt2_watts and vt2_watts > 0:
        ...
        if ratio < 0.65:
            ...
            if result.confidence_level != "low":
                ...
                result.prescriptions.append("⚠️ Zalecenie niepewne z powodu niskiej jakości danych.")
        elif ratio > 0.85:
            result.diagnostics.append(
                f"{qualifier}Wysoka baza aerobowa: VT1 blisko VT2 (>85%). Profil 'Diesel'."
            )
            if result.confidence_level != "low":
                result.prescriptions.append("Sugestia: Trening spolaryzowany - interwały VO2max.")
            else:
                result.prescriptions.append("⚠️ Zalecenie niepewne z powodu niskiej jakości danych.")
        else:
            result.diagnostics.append(
