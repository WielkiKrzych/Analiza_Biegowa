...
def calculate_heat_strain_index(df_pl: Union[pd.DataFrame, Any]) -> pd.DataFrame:
    ...
    df = df.copy()
    df["hsi"] = (
        (5 * (df[core_col] - 37.0) / 2.5) + (5 * (df["heartrate_smooth"] - 60.0) / 120.0)
    ).clip(0.0, 10.0)
...
def calculate_thermal_decay(df_pl: Union[pd.DataFrame, Any]) -> dict:
    ...
    if not all([pwr_col, hr_col, temp_col]):
        return {
            ...
            "message": "Brak wymaganych kolumn (Moc, HR, Temp)",
        }
    ...
    if len(df_act) < 300:  # Min 5 minutes of data
        return {
            ...
            "r_squared": 0,
            "is_significant": False,
            "message": "Zbyt mało danych aktywnych (>37°C)",
        }
    ...
    return {
        ...
        "message": f"Spadek o {abs(decay_pct):.1f}% na każdy +1°C"
        if decay_pct < 0
        else "Stabilna termoregulacja",
    }
...
def predict_thermal_performance(
    ...
    hsi_estimated = min(
        10, max(0, (target_temp - 37.0) / 2.5 * 5 + (hr_at_threshold - 100) / 100 * 5)
    )
...
def calculate_thermal_drift_rate(
    ...
    if abs_drift_h < 0.5:
        ...
        interpretation = (
            f"Thermal drift of {drift_c_per_hour:+.2f} C/h indicates good heat adaptation."
        )
