...
class ResolvedMetric:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {"value": self.value, "source": self.source.value, "confidence": self.confidence}
...
def resolve_metric(
    ...
    if manual is not None and manual != 0:
        return ResolvedMetric(
            value=manual, source=MetricSource.MANUAL, confidence=manual_confidence, name=name
        )

    # AUTO is fallback
    if auto is not None and auto != 0:
        return ResolvedMetric(
            value=auto, source=MetricSource.AUTO, confidence=auto_confidence, name=name
        )

    # MISSING - no valid value
    return ResolvedMetric(value=None, source=MetricSource.MISSING, confidence=0.0, name=name)


def resolve_all_thresholds(
    manual_overrides: Dict[str, Any], auto_values: Dict[str, Any]
...
def apply_manual_overrides_to_thresholds(
    thresholds_dict: Dict[str, Any], manual_overrides: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    mappings = [
        (
            ...
            "manual_vt1_watts",
            lambda t: (
                t.get("vt1", {}).get("midpoint_watts")
                if isinstance(t.get("vt1"), dict)
                else t.get("vt1_watts")
            ),
        ),
        (
            "vt2_watts",
            "manual_vt2_watts",
            lambda t: (
                t.get("vt2", {}).get("midpoint_watts")
                if isinstance(t.get("vt2"), dict)
                else t.get("vt2_watts")
            ),
        ),
        (
            "vt1_hr",
        ...
        (
            ...
            lambda t: t.get("cci_breakpoint") or t.get("cci_breakpoint_watts"),
        ),
        (
            "ve_breakpoint",
            "ve_breakpoint_manual",
            lambda t: t.get("ve_breakpoint") or t.get("ve_breakpoint_watts"),
        ),
        (
            "reoxy_halftime",
            "reoxy_halftime_manual",
            lambda t: t.get("reoxy_halftime") or t.get("halftime_reoxy_sec"),
        ),
    ]
...
def format_power_dual(watts: float, reference_watts: float, reference_name: str = "FTP") -> str:
...
def format_power_range_dual(
    low_watts: float, high_watts: float, reference_watts: float, reference_name: str = "FTP"
) -> str:
    ...
    return (
        f"{int(low_watts)}–{int(high_watts)} W ({int(pct_low)}–{int(pct_high)}% {reference_name})"
    )
