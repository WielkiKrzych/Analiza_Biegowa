...
def _get_time_axis_config(time_max_min: float, title: str = "Czas [hh:mm:ss]") -> dict:
    ...
    return dict(title=title, tickmode="array", tickvals=tick_vals, ticktext=tick_text, tickangle=0)
...
class PulsePowerChartExporter(ChartExporter):
    ...
    def create_figure(self, ctx: ChartContext) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                x=df_pp["time_min"], y=trend, name="Trend", line=dict(color="white", dash="dash")
            )
        )
        ...
        legend_stats = [f"🔋 Avg EF: {avg_eff:.2f} W/bpm", f"📉 Drift: {drift_pct:.1f}%"]
        add_stats_to_legend(fig, legend_stats)
...
class TorqueSmO2ChartExporter(ChartExporter):
    ...
    def create_figure(self, ctx: ChartContext) -> go.Figure:
        ...
        legend_stats = [f"💪 Max Torque: {max_t:.0f} Nm", f"🩸 SmO2 @ Max: {smo2_at_max:.1f}%"]
        add_stats_to_legend(fig, legend_stats)

        fig.update_layout(
            title=self.title, xaxis_title="Torque (Nm)", yaxis_title="SmO2 (%)", **ctx.layout_args
        )
        return fig
...
class SmO2AnalysisChartExporter(ChartExporter):
    ...
    def create_figure(self, ctx: ChartContext) -> go.Figure:
        ...
        if "pace" in df.columns:
            ...
            pace_min = (
                df["pace_smooth"] / 60.0 if "pace_smooth" in df.columns else df["pace"] / 60.0
            )
            fig.add_trace(
                go.Scatter(
                    x=df["time_min"],
        ...
        if s_sec is not None and e_sec is not None:
            ...
            if not df_sel.empty and "smo2_smooth" in df_sel.columns:
                ...
                avg_pace_sel = (
                    df_sel["pace_smooth"].mean()
                    if "pace_smooth" in df_sel
                    else (df_sel["pace"].mean() if "pace" in df_sel else 0)
                )
                ...
                pace_str = (
                    f"{int(avg_pace_min_sel):02d}:{int((avg_pace_min_sel % 1) * 60):02d}"
                    if avg_pace_sel > 0
                    else "--:--"
                )
                legend_stats = [
        ...
        if s_v_sec is not None and e_v_sec is not None:
            ...
            if not df_v.empty and "tymeventilation_smooth" in df_v.columns:
                ...
                slope_v, intercept_v, _, _, _ = stats.linregress(
                    df_v["time"], df_v["tymeventilation_smooth"]
                )
