...
class DataAnonymizer:
    ...
    def get_bracket(self, value: float, brackets: List[tuple]) -> str:
    ...
    def anonymize_session(
        self, df: pd.DataFrame, metrics: dict, age: int, weight: float, gender: str
    ) -> dict:
    ...
    def anonymize_profile(
        self, user_id: str, age: int, weight: float, gender: str, ftp: float, mmp_data: dict
    ) -> AnonymizedProfile:
...
class ComparisonService:
    ...
    def get_ftp_percentile(self, ftp_wkg: float, gender: str = "M") -> PercentileRanking:
        ...
        Returns:
            PercentileRanking
        """
        percentiles = self.FTP_PERCENTILES_MALE if gender == "M" else self.FTP_PERCENTILES_FEMALE
    ...
    def get_vo2max_percentile(
        self, vo2max: float, age: int, gender: str = "M"
    ) -> PercentileRanking:
        ...
        return PercentileRanking(
            metric="VO2max", value=vo2max, percentile=95, category="Elite", sample_size=5000
        )
    ...
    def get_summary_rankings(
        self, ftp: float, weight: float, vo2max: float, age: int, gender: str
