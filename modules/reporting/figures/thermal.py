...
def generate_thermal_chart(
    ...
    if source_df is None or source_df.empty:
        ...
        if time_series and (time_series.get("core_temp") or time_series.get("hsi")):
            ...
            return create_empty_figure(
                "Brak danych źródłowych", "Termoregulacja", output_path, **cfg
            )

    # Resolve columns
    df = source_df.copy()
    temp_col = _find_column(
        df, ["core_temperature_smooth", "core_temperature", "core_temp", "temp", "temperature"]
    )
    ...
    ax1.axhline(
        y=38.5, color="red", linestyle="--", alpha=0.5, linewidth=1, label="Krytyczna (38.5°C)"
    )
    ax1.axhline(
        y=37.5, color="green", linestyle=":", alpha=0.5, linewidth=1, label="Optymalna (37.5°C)"
    )
    ...
    (l2,) = ax2.plot(
        ...
        color="#d62728",
        linestyle=":",
        label="Wsk. Obciążenia Cieplnego",
        linewidth=2,
    )
...
def generate_efficiency_chart(
    ...
    if source_df is None or source_df.empty:
        ...
        if time_series and time_series.get("power_watts") and time_series.get("core_temp"):
            ...
            return create_empty_figure(
                "Brak danych źródłowych", "Spadek Efektywności", output_path, **cfg
            )
    ...
    if not (temp_col and hr_col and pwr_col):
        return create_empty_figure(
            "Brak danych Power/HR/Temp", "Spadek Efektywności", output_path, **cfg
        )
    ...
    ax.set_title(
        "Spadek Efektywności (Cardiac Drift) vs Temperatura", fontsize=title_size, fontweight="bold"
    )
