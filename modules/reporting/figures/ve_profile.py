...
def generate_ve_profile_chart(
    ...
    if source_df is not None and not source_df.empty:
        ...
        ve_col = next(
            (c for c in ["tymeventilation", "ve", "ventilation", "ve_smooth"] if c in df.columns),
            None,
        )
        pace_col = next(
    ...
    if pace_data and vt1_pace_sec:
        for t, p_sec in zip(
            time_data, time_series.get("pace_sec_per_km", time_series.get("pace", [])), strict=False
        ):
    ...
    if pace_data:
        ax2.plot(
            time_min, pace_data, color=get_color("pace"), alpha=0.3, linewidth=1, label="Tempo"
        )
