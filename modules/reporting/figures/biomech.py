...
def generate_biomech_chart(
    ...
    if source_df is not None and not source_df.empty:
        ...
        if "power" in df.columns and "cad_raw" in df.columns:
            ...
            df["torque"] = df.apply(
                lambda row: (
                    row["power"] / (row["cadence"] * 2 * np.pi / 60) if row["cadence"] > 20 else 0
                ),
                axis=1,
            )
        ...
        if power_vals and cadence_vals and time_sec:
            ...
            torque_data = []
            for p, c in zip(power_vals, cadence_data, strict=False):
                t = p / (c * 2 * np.pi / 60) if c > 20 else 0
                torque_data.append(t)

            torque_data = (
                pd.Series(torque_data)
    ...
    if not time_min or not torque_data:
        return create_empty_figure(
            "Brak danych mocy/kadencji dla biomechaniki", "Biomechanika", output_path, **cfg
        )

    # Use the prepared data
    df_plot = pd.DataFrame(
        {"time_min": time_min, "cadence": cadence_data, "torque_smooth": torque_data}
    )
    ...
    fig.text(
        0.01,
        0.01,
        f"ID: {session_id} | Biomechanika (Torque vs Cadence)",
        ha="left",
        va="bottom",
        fontsize=8,
...
def generate_torque_smo2_chart(
    ...
    if not all([power_vals, cadence_vals, smo2_vals]):
        return create_empty_figure(
            "Brak danych (Power, Cad, SmO2) dla Torque-SmO2",
            "Fizjologia Okluzji",
            output_path,
            **cfg,
        )
    ...
    df_calc = pd.DataFrame({"power": power_vals, "cadence": cadence_vals, "smo2": smo2_vals})
    ...
    if len(df_clean) < 20:
        return create_empty_figure(
            "Za mało punktów danych do analizy Torque-SmO2",
            "Fizjologia Okluzji",
            output_path,
            **cfg,
        )
    ...
    fig.text(
        0.01,
        0.01,
        f"ID: {session_id} | Fizjologia Okluzji (Analiza Koszykowa)",
        ha="left",
        va="bottom",
        fontsize=8,
        color=get_color("secondary"),
