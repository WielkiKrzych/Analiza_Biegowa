...
def generate_cp_curve_chart(
    report_data: Dict[str, Any], config: Optional[Any] = None, output_path: Optional[str] = None
) -> bytes:
    ...
    if cp_watts > 0 and w_prime_j > 0:
        ...
        # CP horizontal line
        ax.axhline(
            y=cp_watts, color=get_color("cp"), linewidth=1.5, linestyle=":", alpha=0.7, zorder=2
        )
    ...
    # Title
    ax.set_title("Power-Duration Curve (PDC)", fontsize=title_size, fontweight="bold", pad=15)
...
def _interpolate_power(
    durations: List[float], powers: List[float], target_sec: float
) -> Optional[float]:
