...
def generate_ramp_profile_chart(
    ...
    # Power trace (Axis 1)
    (l1,) = ax.plot(
        time_min, power_data, color=get_color("power"), linewidth=1.5, label="Moc", zorder=3
    )
    ...
    if vt1_range:
        ...
        ax.axhline(
            y=vt1_watts, color=get_color("vt1"), linewidth=1, linestyle=":", alpha=0.7, zorder=2
        )
    ...
    if vt2_range:
        ...
        # Add center line for clarity
        ax.axhline(
            y=vt2_watts, color=get_color("vt2"), linewidth=1, linestyle=":", alpha=0.7, zorder=2
        )
