...
def build_page_executive_verdict(
    ...
    hero_content = [
        ...
        [
            Paragraph(
                f"<font size='8' color='#BDC3C7'>Confidence score: {confidence_score:.2f} | Źródła: VO₂max ({vo2max_source}), SmO₂, HR coupling, Core Temp</font>",
                styles["center"],
            )
        ],
    ]
    ...
    thermo_items.append(
        f"• Max Core Temp: {max_core_temp:.1f} °C" if max_core_temp > 0 else "• Max Core Temp: ---"
    )
    thermo_items.append(f"• Peak HSI: {peak_hsi:.1f}" if peak_hsi else "• Peak HSI: ---")
    thermo_items.append(
        f"• Cardiac Drift (ΔEF): {ef_delta_pct:+.1f}%" if ef_delta_pct else "• Cardiac Drift: ---"
    )
    thermo_items.append(
