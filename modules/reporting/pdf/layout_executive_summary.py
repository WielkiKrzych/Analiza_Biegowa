...
def build_page_executive_summary(
    executive_data: Dict[str, Any], metadata: Dict[str, Any], styles: Dict
) -> List:
    ...
    # Title row
    elements.append(
        Paragraph("<font size='14'>5.2 PODSUMOWANIE FIZJOLOGICZNE</font>", styles["center"])
    )
    ...
    verdict_content = [
        Paragraph(
            f"<font size='14'><b>{limiter_icon} DOMINUJĄCY LIMITER: {limiter_name}</b></font>",
            styles["heading"],
        ),
    ...
    idx_color = (
        HexColor("#D5F5E3")
        if agreement_idx >= 0.8
        else (HexColor("#FCF3CF") if agreement_idx >= 0.5 else HexColor("#FADBD8"))
    )
    idx_content = [
    ...
    breakdown_rows = []
    for key, label in [
        ("ve_stability", "VE Stability"),
        ("hr_lag", "HR Response"),
        ("smo2_noise", "SmO₂ Quality"),
        ...
        breakdown_rows.append(
            [
                ...
                Paragraph(
                    f"<font size='9' color='{bar_color}'><b>{val}%</b></font>", styles["body"]
                ),
            ]
        )
    ...
    elements.append(
        Paragraph(
            f"<font size='9' color='#7F8C8D'>Ogranicza: <b>{limiting_factor}</b></font>",
            styles["body"],
        )
    )
    elements.append(Spacer(1, 6 * mm))
    ...
    for i, card in enumerate(training_cards[:3], 1):
        ...
        card_content = [
            ...
            Paragraph(
                f"<font size='9' color='#7F8C8D'>Spodziewany efekt: {response}</font>",
                styles["body"],
            ),
            Paragraph(
                f"<font size='8' color='{risk_color}'>Ryzyko: {risk_label}</font>", styles["body"]
            ),
        ]
        ...
        if constraint:
            card_content.append(
                Paragraph(
                    f"<font size='8' color='#E67E22'><b>{constraint}</b></font>", styles["body"]
                )
            )
