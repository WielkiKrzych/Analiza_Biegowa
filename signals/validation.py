...
def detect_missing_data(
    series: pd.Series, max_missing_ratio: float = 0.2
    ...
    if series is None or len(series) == 0:
        return ValidationWarning(
            code="EMPTY_SIGNAL", message="Signal is empty or None", severity=Severity.ERROR
        )
...
def detect_artifacts(
    series: pd.Series, z_threshold: float = 3.0, spike_threshold: float = 0.5
...
def check_minimum_length(series: pd.Series, min_length: int = 30) -> Optional[ValidationWarning]:
    """
    ...
    if series is None:
        return ValidationWarning(
            code="NULL_SIGNAL", message="Signal is None", severity=Severity.ERROR
        )
...
def check_data_range(
    series: pd.Series, valid_range: Tuple[float, float] = None, column_name: str = "signal"
) -> Optional[ValidationWarning]:
    """
...
def validate_signal(
    ...
    try:
        ...
        # Check for artifacts
        artifact_indices, artifact_warning = detect_artifacts(series, z_threshold, spike_threshold)
        if artifact_warning:
    ...
    return ValidationResult(
        is_valid=is_valid, warnings=warnings, stats=stats, artifact_indices=artifact_indices
    )
