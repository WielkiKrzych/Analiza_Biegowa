...
class SignalQualityFlags:
    ...
    @classmethod
    def from_series(
        cls, series: pd.Series, min_valid_ratio: float = 0.8, max_noise_level: float = 0.3
    ) -> "SignalQualityFlags":
        """Compute quality flags from a pandas Series."""
        if series is None or len(series) == 0:
            return cls(
                valid_ratio=0.0, gap_count=0, max_gap_duration=0, noise_level=1.0, is_usable=False
            )
...
def exponential_smooth(series: pd.Series, alpha: float = 0.3, adjust: bool = True) -> SeriesResult:
    """
...
def detrend_polynomial(series: pd.Series, degree: int = 2) -> SeriesResult:
    """
    ...
    return SeriesResult(
        ...
        quality=quality,
        method="detrend_polynomial",
        parameters={"degree": degree, "coefficients": [round(c, 6) for c in coeffs]},
    )
...
def interpolate_gaps(
    ...
    return SeriesResult(
        ...
        quality=quality,
        method="interpolate_gaps",
        parameters={"method": method, "max_gap": max_gap, "gaps_filled": int(gaps_filled)},
    )
