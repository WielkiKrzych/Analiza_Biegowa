...
def test_repeatability():
    ...
    # VT1 = 200, 202, 198
    sessions = [{"vt1": 200, "tau": 30}, {"vt1": 202, "tau": 32}, {"vt1": 198, "tau": 29}]
    ...
    res = comp["vt1"]
    print(
        f"Comparison: Current={res['current']}, Diff={res['pct_diff']}%, Sig={res['is_significant']}"
    )

    assert res["is_significant"], "Should be significant change"
    assert "Change" in res["status"], "Status text should reflect change"

    # 3. Create Unstable Metric (High CV)
    # Tau = 30, 45, 20
    sessions_unstable = [{"tau": 30}, {"tau": 45}, {"tau": 20}]
