...
from modules.calculations.kinetics import detect_smo2_trend, normalize_smo2_series
...
def test_trend_detection():
    ...
    assert "Szybka deoksygenacja" in res_a["category"] or "deoksygenacja" in res_a["category"], (
        "Failed to detect Deoxygenation"
    )
    ...
    assert "Równowaga" in res_b["category"] or "Stabilny" in res_b["category"], (
        "Failed to detect Equilibrium"
    )
    ...
    assert "Reoksygenacja" in res_c["category"] or "reoksygenacja" in res_c["category"].lower(), (
        "Failed to detect Reoxygenation"
    )
